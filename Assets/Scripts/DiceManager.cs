...
public class DiceManager : MonoBehaviour
{
    ...
    public void Start()
    {
        SetDiceRange(PlayerPrefs.GetInt("DiceRange", 6));
        SetDiceAnimation((ANIM)PlayerPrefs.GetInt("DiceAnimation", 0));
        
        int savedDiceCount = PlayerPrefs.GetInt("DiceCount", 1);
        if (rollController != null && savedDiceCount > 0)
        {
            rollController.SpawnAmount(savedDiceCount);
        }

        bool isNumeric = PlayerPrefs.GetInt("DiceNumeric", 0) == 1;
        if (rollController != null && rollController.isOnlyNumber != isNumeric)
        {
            rollController.SetDotOrNumber(); // Toggle to match saved state
        }
        UpdateDiceTypeUI();
    ...
    public void AddDice()
    {
        ...
        PlayerPrefs.SetInt("DiceCount", rollController.spawnedDice.Count);
        PlayerPrefs.Save();
    ...
    public void SetDiceOnlyNumaric()
    {
        ...
        PlayerPrefs.SetInt("DiceNumeric", rollController.isOnlyNumber ? 1 : 0);
        PlayerPrefs.Save();
        UpdateDiceTypeUI();
    }
